...
For an ordinary first setup and join, prefer the single invitation generated by
the administrator:
...
address, TLS name, controller ID, SPKI pin, network ID, and both single-use
tokens. The client creates its identity and configuration, then enrolls and
joins. With an existing configuration, the invitation's controller trust must
match the stored trust. Invitations expire after one hour by default and contain
