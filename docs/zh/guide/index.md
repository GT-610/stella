...
和 macOS 原生客户端数据平面已经实现。Windows 从预安装的驱动包为每个网络自动创建持久
TAP-Windows 适配器；macOS 使用 Stella 自有后端，通过持久内置 feth pair、BPF 收包和
AF_NDRV 发包，并由本地特权 helper 隔离 root 权限。
...
只需安装 TAP-Windows Adapter V9 驱动包，客户端会为每个网络自动创建或复用一个托管
适配器；macOS 每个网络需要一个未占用的持久 feth pair，以及
