...
"@
    if (-not $text.Contains($needle)) {
        throw "generated client configuration is missing $needle"
    }
    [IO.File]::WriteAllText($ConfigPath, $text.Replace($needle, $endpoint), [Text.UTF8Encoding]::new($false))
}
...
try {
    ...
    Wait-Log $leftStderr 'Windows data plane is active' $leftProcess
    Wait-Log $rightStderr 'Windows data plane is active' $rightProcess
