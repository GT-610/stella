...
struct JoinArgs {
    ...
    #[arg(
        ...
        conflicts_with_all = ["network", "token", "enrollment_token"]
    )]
    ...
    #[arg(long, value_name = "PATH", requires = "invitation")]
...
async fn join_network(config_path: &Path, args: &JoinArgs, output: &mut dyn Write) -> Result<()> {
    ...
    prepare_invitation_configuration(config_path, args, output)?;
    let config = ClientConfig::load(config_path).context("could not load client configuration")?;
    let network_id = join_network_id(args)?;
    ...
    let invitation_join_token = args
        .invitation
        .as_ref()
        .map(|value| BearerCredential::from_bytes(*value.0.join_token()));
    let invitation_enrollment_token = args
        .invitation
        .as_ref()
        .map(|value| BearerCredential::from_bytes(*value.0.enrollment_token()));
...
fn join_network_id(args: &JoinArgs) -> Result<NetworkId> {
    match (&args.invitation, args.network) {
        (Some(invitation), None) => Ok(invitation.0.network_id()),
...
fn prepare_invitation_configuration(
    ...
    let Some(invitation) = args.invitation.as_ref().map(|value| &value.0) else {
        return Ok(());
    };
...
mod tests {
    ...
    use super::{
        ...
        prepare_invitation_configuration, reconnect_cap, remove_network_intent, CliCredential,
        CliInvitation, InitArgs, JoinArgs, MAXIMUM_RECONNECT_DELAY,
    };
    #[cfg(any(windows, target_os = "macos"))]
    ...
    #[cfg(target_os = "macos")]
    use super::{validate_intent_compatibility, validate_join_tap, Cli, Command};
    ...
    fn invitation_creates_matching_configuration_and_rejects_trust_changes() {
        ...
        prepare_invitation_configuration(&config_path, &args, &mut output)
        ...
        assert!(
            prepare_invitation_configuration(&config_path, &conflicting_args, &mut Vec::new())
                .is_err()
        );
